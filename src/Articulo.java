...
public class Articulo {
	...
	public void store() {
		...
		try {
			...
			System.out.println(sql); // Muestro en una consola el error para en
										// caso de que falle poder arreglarlo
			e.printStackTrace();
		}
	...
	/**
	 * Borrar una tupla. Paspo el id_articulo del articulo que quiero borrar
	 * como parámetro
	 * 
	 * @param id_articulo
	 */
	...
	/**
	 * Método para modificar artículos. Se pasan el id como parametro para
	 * seleccionar el articulo y el campo a modificar junto con su valor
	 * 
	 * @param id_articulo
	 * @param campo
	 * @param valor
	...
	public void updateAll(int id_articulo, String nombre, Double precio, String descripcion, String volumen,
			String embalaje) {
	...
	/**
	 * Buscar por id, que paso como parametro. El metodo asigna a cada atributo
	 * de la clase su valor en la bbdd en caso de encontrar una correspondecia
	 * 
	 * @param id_articulo
	 * @return found Devuelve true si ha encontrado el articulo, y false en caso
	 *         contrario
	 */
	public boolean find(int id_articulo) {
		...
		try {
			...
			if (rs.next()) {
				// MODIFICAMOS ESTA CLASE PARA AÑADIR EL ID Y PODER RECUPERARLO
	...
	String sql = "INSERT INTO `articulos`(`nombre`, `precio`, `descripcion`, `volumen`, `embalaje`, `cif`, `id_categoria`, `stock`) "
			+ "VALUES ('"+this.nombre+"','"+this.precio+"','"+this.descripcion+"','"+this.volumen+"','"+this.embalaje+"','"+this.cif+"','"+this.id_categoria+"','"+this.stock+"')";
		try {
			Conexion c = new Conexion();
			c.modificar(sql);
		} catch (ClassNotFoundException | SQLException e) {
