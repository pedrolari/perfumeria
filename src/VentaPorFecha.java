...
public class VentaPorFecha extends JInternalFrame implements ActionListener {
	...
	VentaPorFecha() {
		...
		principal.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(new Color(41, 53, 65), 1),
				"VENTA POR FECHA", TitledBorder.LEFT, TitledBorder.TOP, new Font(null, Font.BOLD, 25),
				new Color(41, 53, 65)));
		...
		if (arg0.getSource().equals(comprobar)) {
			...
			if (comprobarFecha(date1) == true) {
				if (d1.after(c.getTime())) {
					JOptionPane.showMessageDialog(this, "La fecha no puede ser posterior a la de hoy.",
							"Cliente por fecha", JOptionPane.INFORMATION_MESSAGE);
				} else {
	...
	public boolean comprobarFecha(JDateChooser jd) {
		...
		if (jd.getDate() != null) {
			...
			JOptionPane.showMessageDialog(this, "No inserto la fecha correctamente", "Cliente por fecha",
	...
	public void cargarVentas(Date d) {
		...
		try {
			...
			while (rs.next()) {
				enc = true;
				modelo.addRow(new Object[] { rs.getString(2), rs.getString(3), "" + rs.getDate(4),
						"" + df.format(rs.getDouble(5)) + "€" });
			}
		} catch (ClassNotFoundException | SQLException e) {
			e.printStackTrace();
		}
		if (enc == false) {
			JOptionPane.showMessageDialog(this, "No se han encontrado ventas con esa fecha.", "Cliente por fecha",
					JOptionPane.INFORMATION_MESSAGE);
