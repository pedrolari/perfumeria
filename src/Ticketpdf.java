...
public class Ticketpdf {
	...
	private void crearTicket(String user, int idVent) throws SQLException, JRException, ClassNotFoundException {
		...
		ResultSet rs = stm.executeQuery("SELECT * FROM empleados,lineas_de_ventas,articulos,ventas WHERE empleados.user like '"+user+"' "
		...
		while(rs.next()==true){
			...
			Ticket t = new Ticket(empleado,rs.getInt("lineas_de_ventas.id_venta"),rs.getInt("lineas_de_ventas.id_articulo"),rs.getString("articulos.nombre")
					,rs.getInt("lineas_de_ventas.precio"),rs.getInt("lineas_de_ventas.cantidad"),total,rs.getInt("ventas.total_pedido"));
            listaTicket.add(t);
		}
           

        JasperReport reporte = (JasperReport) JRLoader.loadObjectFromFile("reporte.jasper");
        JasperPrint jasperPrint = JasperFillManager.fillReport(reporte, null, new JRBeanCollectionDataSource(listaTicket));
        // </editor-fold>
